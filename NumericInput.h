...
class NumericInput: public PrimitiveInput {
    ...
    uint8_t flags;
    public:
    NumericInput(uint16_t colorb, uint16_t colorbp, uint16_t tcolor, uint16_t x, uint16_t y, uint16_t w, uint16_t h, GFX4d *gfx, void (*callback)(int, int), int textsize, int id, uint8_t flags);
    ~NumericInput();
